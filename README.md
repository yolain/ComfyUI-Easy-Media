...
| `sampling_mode` | `single`, `dual`, or `selflift`; SelfLift expands `model_hires`, `transition_ratio`, and `lowres_scale`; high-resolution tiling uses the shared `enabled_tiling` control. Pixel/VAE correction is disabled for ordinary segments and uses an internal conservative preset for context continuation. SelfLift also saves separate low- and high-resolution context lineages |
| `model_hires` | Optional MODEL input shown in SelfLift mode. When disconnected, both stages use the original model. When connected, only high-resolution denoising after the lift uses this model and its complete LoRA stack. Use the same H3 architecture, latent format, and noise/audio scales; CLIP, VAE, and the sigma schedule stay unchanged |
