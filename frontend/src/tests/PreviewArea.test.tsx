...
describe('PreviewArea', () => {
  ...
  it('overlays active subtitles on the video preview frame', () => {
    ...
    const videoStage = screen.getByTestId('multitrack-video-stage')
    expect(videoStage.className).toContain('absolute')
    expect(videoStage.className).toContain('inset-0')
    expect(videoStage.parentElement?.className).toContain('relative')
    expect(videoStage.parentElement?.className).toContain('min-h-0')
    expect(videoStage.parentElement?.className).toContain('h-auto')
    expect(videoStage.parentElement?.className).toContain('max-h-full')
    expect(videoStage.parentElement?.style.aspectRatio).toBe(videoStage.style.aspectRatio)
    expect(videoStage.parentElement?.className).not.toContain('transition')
