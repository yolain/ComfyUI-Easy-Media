...
class LTXMultiTrackEncode(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            inputs=[
                ...
                io.Boolean.Input("i2v_inplace_enabled", default=False),
                io.Float.Input("i2v_inplace_strength", default=0.7, min=0.0, max=1.0, step=0.01),
                io.Int.Input("i2v_inplace_index", default=0, min=0, step=1),
    ...
    def execute(
        ...
        i2v_inplace_enabled: list[bool] | bool = False,
        i2v_inplace_strength: list[float] | float = 0.7,
        i2v_inplace_index: list[int] | int = 0,
        ...
        if bool(_first_value(i2v_inplace_enabled, False)):
            images = _flatten_images(image)
            index = int(_first_value(i2v_inplace_index, 0))
            if not 0 <= index < len(images):
                raise ValueError(
                    f"i2v_inplace_index {index} is outside the available image range (count: {len(images)})"
                )
            processed = LTXVPreprocess.execute(
                images[index],
                int(_first_value(img_compression, 18)),
            )[0]
            video_latent = LTXVImgToVideoInplace.execute(
                selected_vae,
                processed,
                video_latent,
                float(_first_value(i2v_inplace_strength, 0.7)),
                False,
            )[0]
