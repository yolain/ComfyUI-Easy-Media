...
- 用户明确要求实际生成时，上传素材并提交到指定 ComfyUI；未给地址时使用 `http://127.0.0.1:8188`。
...
- MiniMax 项目中，是否要把某个音频片段设为所有任务共用的说话人/音色参考。
- 是否需要实际上传或执行，以及 ComfyUI 服务地址；未提供地址时使用 `http://127.0.0.1:8188`。
...
- `speaker_reference` 是否只出现在音频片段，且同一音轨最多一个。
- 修改前后的节点数、连线和非目标节点内容是否保持不变。

上传媒体和提交 ComfyUI 队列属于外部状态变更。“生成一个工作流”只生成 JSON；只有用户明确要求运行、执行、开始生成或提交队列时才执行。用户未提供 ComfyUI 地址时默认使用 `http://127.0.0.1:8188`。
