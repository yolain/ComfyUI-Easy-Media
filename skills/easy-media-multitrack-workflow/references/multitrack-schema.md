...
1. 用户提供视频用于主体或局部替换且未要求移除/替换原声时，锁定该 video track。
2. 用户明确指定“锁定”“主音轨”“沿用原声”“不要重生成音频”时，使用其指定的 audio/video 轨。
3. MV、音乐视频、歌词视频、卡点视频、舞蹈视频、按歌曲节拍或完整歌曲时长生成画面时，将歌曲/配乐主轨锁定。
4. 旁白、播客或对白是视频时长与镜头节奏的唯一基准时，也可锁定该旁白主轨。
5. 普通背景音乐、环境声、音效、角色参考音频或临时占位音频不自动视为主锁定音轨，除非用户语义表明它负责驱动生成。
6. 已有一条锁定媒体轨且用户未要求更换时，优先保留；但仍要检查它是否与本次要生成的任务片段范围相交。
...
- 锁定轨至少包含一个与轨道类型一致的媒体片段：audio track 对应 `media_type: "audio"`，video track 对应 `media_type: "video"`；锁定视频原声时还要确认源视频确实包含音轨。
- 锁定媒体片段应与本次每个目标 task segment 的 `[start_frame, end_frame)` 相交；不相交时，先调整时间线或询问用户。
- 作为主音频使用时通常保持 `muted: false`。不要为了“锁定”而修改无关的 solo、音量或轨道编辑锁状态。
- 如果上下文明确不需要保留任何原音频，应确保所有 audio/video track 的 `audio_locked` 为 `false`。

## 公用媒体

`shared_reference` 是任务图片项以及音频/视频片段 `content` 中的布尔字段。当多个上下文任务分段需要使用同一张图片、同一段音频或同一段视频作为参考时，将对应媒体设置为公用媒体；它不要求在时间线上与每个任务片段重叠。
...
- 用户说“设为公用媒体”“所有上下文分段都参考这张图/这段音频/这个视频”或表达相同语义时，标记其指定媒体；不要只因文件名相同就自动共享。
- 任务图片的标记写在 `content.images[]` 项上，可以有多张公用图片；它们会排在每个任务的私有图片之前，公用和私有图片总数仍最多为 9。
- 音频/视频的标记写在对应媒体片段 `content` 上，同一音频轨或视频轨最多一个；切换时把该轨其他片段显式设为 `false`。
- 公用音频从源文件开头使用，最多 15 秒；公用视频使用该源视频作为各任务的参考视频。
- 公用媒体是跨任务参考，不等于时间线原声复用。不要仅因为音频设为公用就同时设置 `audio_locked`；只有用户还要求原样保留音频并用它驱动时间线时，才分别评估 `audio_locked`。
- 若用户既要某段音频/视频作为公用参考，又要它按普通时间线语义输出，应使用独立轨道或片段表达两个角色，并在交付摘要中说明。
- `speaker_reference` 是旧版音频参考字段，只用于读取和迁移旧工作流；新建或更新工作流统一写 `shared_reference`。
- 多个候选无法消歧时，列出媒体项或片段 ID、轨道名称、文件名、时长或帧范围，请用户指定；不要按第一项或最长片段选择。

## 自动编排决策

- 用户给出明确镜头/片段长度时优先采用；否则按素材时长或提示词中的时间信息推导，并说明推导。
- 主体或局部视频替换模式在用户未指定片段长度时，固定按 10 秒连续拆分 task segment，最后一段使用剩余时长。
...
- 对主体替换视频、MV 或其他主音频驱动项目，在完成时间线编排后锁定唯一的 audio/video 轨，并复核它覆盖目标任务范围。
