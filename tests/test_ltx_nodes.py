...
def _execute_encode(module, **overrides):
    values = {
        ...
        "vae": ["vae"],
        "audio_vae": ["audio-vae"],
        "image": [torch.stack([torch.zeros(2, 2, 3), torch.ones(2, 2, 3)])],
...
def test_ltx_multitrack_encode_schema_matches_confirmed_interface(monkeypatch):
    ...
    assert [item.name for item in schema.inputs] == [
        "model", "clip", "vae", "audio_vae", "image", "audio",
        "local_prompt", "global_prompt", "epsilon", "width", "height", "frame_rate",
        "video_length", "half_latent_size",
        "i2v_inplace_enabled", "i2v_inplace_strength", "i2v_inplace_index", "img_compression",
    ]
    assert schema.inputs[4].kwargs["optional"] is True
    assert schema.inputs[5].kwargs["optional"] is True
    assert schema.inputs[-2].kwargs["default"] == 0
    assert schema.inputs[-1].kwargs["default"] == 18
...
def test_encode_rejects_an_out_of_range_i2v_image_index(monkeypatch):
    module, _ = _load_ltx_module(monkeypatch)

    with pytest.raises(ValueError, match="i2v_inplace_index"):
        _execute_encode(module, i2v_inplace_index=[3])
...
def test_ltx_encode_execute_defaults_match_schema_defaults(monkeypatch):
    ...
    assert signature.parameters["i2v_inplace_enabled"].default is False
